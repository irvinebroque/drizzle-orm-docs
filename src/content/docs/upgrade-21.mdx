...
#### 2. Update your `drizzle.config.ts` file:
 ...
 - Add `driver` to `drizzle.config.ts` ONLY if you are using `aws-data-api`, `turso`, `d1-object`, legacy `d1-http`, or `expo`. Otherwise, you can remove the `driver` from `drizzle.config.ts`.
...
export default defineConfig({
    ...
    driver: "turso", // optional and used only if `aws-data-api`, `turso`, `d1-object`, legacy `d1-http` or `expo` are used
    dbCredentials: {
        url: ""
    }
})
...
- `driver` has become optional and will have a specific driver, each with a different configuration of `dbCredentials`. Available drivers are:
  - `aws-data-api`
  - `turso`
  - `d1-object` - Cloudflare D1 application objects
  - `d1-http` - legacy Cloudflare D1 HTTP API
  - `expo`
