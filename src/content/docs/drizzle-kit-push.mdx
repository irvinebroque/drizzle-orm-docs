...
<Callout type="warning">
Cloudflare D1 application objects use `driver: "d1-object"` for generating bundled migrations. `push` does not connect directly to a D1 application object; run generated migrations through a primary-only Durable Object method instead. The `d1-http` driver is for legacy D1 database binding projects.
</Callout>
