...
<Callout type="warning">
Cloudflare D1 with `driver: "d1-object"` uses generated bundled migrations. `push` does not connect directly; run generated migrations through a primary-only object method instead. The `d1-http` driver is for legacy D1 database binding projects.
</Callout>
