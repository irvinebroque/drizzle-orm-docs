...
as for exceptions like `aws-data-api`, `pglight`, `d1-object`, and `d1-http` - you will have to explicitly specify `driver` param.

<Callout type="warning">
Cloudflare D1 with `driver: "d1-object"` uses generated bundled migrations. `push` does not connect directly; run generated migrations through an object method that calls `applyDrizzleMigrations()` instead. The `d1-http` driver is for legacy D1 database binding projects.
</Callout>
