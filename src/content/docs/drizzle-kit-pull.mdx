...
<Callout type="warning">
Cloudflare D1 with `driver: "d1-object"` uses generated bundled migrations. `pull` does not connect directly. The `d1-http` driver is for legacy D1 database binding projects.
</Callout>

<CodeTabs items={["AWS Data API", "PGLite", "Cloudflare D1 HTTP (legacy)"]}>
```ts {6}
import { defineConfig } from "drizzle-kit";
